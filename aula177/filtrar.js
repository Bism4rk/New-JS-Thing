...
const preencherDGV=(endpoint)=>{
    dados.innerHTML = ""
    this.endpoint =`http://localhost:1880/pesquisartodoscontatos`
    fetch(endpoint)
    .then(res=>res.json())
    .then(res=>{
        res.forEach(el => {
...
preencherDGV(`http://localhost:1880/pesquisartodoscontatos`)
...
btn_filtrar.addEventListener('click', (evt)=>{
    if(f_filtronome.value == ""){
        ...
        preencherDGV(`http://localhost:1880/filtrar/${f_filtronome.value}`)
    }
})